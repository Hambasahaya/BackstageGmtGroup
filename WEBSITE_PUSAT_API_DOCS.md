...
SSO_CLIENTS=website_a=https://website-a.com/sso/callback,website_utama=https://website-utama.com/sso/callback
```
...
```text
https://website-utama.com/sso/callback?code=<one-time-code>&state=<state>
```
...
```json
{
  ...
  "redirect_url": "https://website-a.com/sso/callback?code=one-time-code&state=random-state"
}
```
...
Response preorder list/detail bisa menyertakan field payment:
...
Untuk membuat atau mengambil link pembayaran manual:
...
Frontend cukup menampilkan status terbaru dari API list/detail PO. Webhook Midtrans langsung masuk ke backend:
...
Catatan: backend otomatis membuat link Midtrans saat PDF dicetak jika link belum ada, lalu link tersebut muncul di PDF.
